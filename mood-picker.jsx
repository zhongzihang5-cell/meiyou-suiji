...
function buildMoodStreakInsight(currentMood){
  const score = moodScoreOf(currentMood);
  if(score >= 4) return '连续 2 天情绪较好';
  if(score === 3) return '近期情绪比较平稳';
  return '最近情绪有些低落，多照顾自己';
}
...
function createMoodQuickEntry(moods, history){
  ...
  const { points, time } = buildTodayMoodPoints(history, primary);
  const score = moodScoreOf(primary);
